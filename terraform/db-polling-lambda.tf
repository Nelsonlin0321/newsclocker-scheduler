...
resource "aws_iam_role" "iam_for_lambda" {
  name               = "${locals.db_polling_lambda_name}-iam-role"
  assume_role_policy = data.aws_iam_policy_document.assume_role.json
}

resource "aws_cloudwatch_log_group" "lambda_log_group" {
  name              = "/aws/lambda/${locals.db_polling_lambda_name}"
...
resource "aws_iam_policy" "lambda_logging_policy" {
  name        = "${locals.db_polling_lambda_name}_logging"
  path        = "/"
...
resource "aws_iam_policy" "lambda_secrets_policy" {
  name        = "${locals.db_polling_lambda_name}-secrets"
  path        = "/"
...
resource "aws_lambda_function" "newsclocker_db_polling" {
  function_name = "${locals.db_polling_lambda_name}"
